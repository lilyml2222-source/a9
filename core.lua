-- CORE.LUA - Main Logic Functions (AUTO WALK ONLY)
local Core = {}
    ...
    while Config.isPlaying do
        ...
        for i = Config.SavedCP, #Config.TASDataCache do
            ...
            for f = Config.SavedFrame, #data do
                ...
                if frame.VEL then
                    ...
                    if isClimbing then
                        Root.AssemblyLinearVelocity = vel * 0.8
                    else
        ...
        if Config.isPlaying then
            if Config.isLooping then
                if Config.autoRespawnLoop then
                    Core.ResetCharacter()
                    ...
                    if success then
                        ...
                        if Core.WindUI then
                            Core.WindUI:Notify({
                                Title="Auto Respawn", 
                                Content="Respawned! Restarting loop...", 
                                Duration=2
                            })
                        end
